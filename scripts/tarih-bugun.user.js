...
(function() {
    ...
    function addDateButtons() {
        document.querySelectorAll('input[type="date"]').forEach(input => {
            // Buton zaten varsa ekleme - güçlendirilmiş kontrol
            const existingButton = input.parentNode.querySelector('.date-today-btn');
            if (existingButton) return;

            // Alternatif kontrol: nextElementSibling kontrolü
    ...
    const observer = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                // Eklenen node'ların bizim butonumuz olmadığını kontrol et
                let shouldAdd = false;
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1 && !node.classList.contains('date-today-btn')) {
                        shouldAdd = true;
                    }
                });
                if (shouldAdd) {
